...
template <class T>
static int max(T a, T b) { return (a > b) ? a : b; }
template<class T>
...
public:
    ...
    AVLNode<T>* find_min(AVLNode<T>* root) const;
    ...
    int get_nodes_counter() const { return nodes_counter; }
...
template <class T>
AVLNode<T>* AVLTree<T>::find_min(AVLNode<T>* root) const {
    if(nodes_counter <= 0)
        return nullptr;
    if(root == nullptr)
        return nullptr;
