...
class AVLNode {
public:
    explicit AVLNode(const T& value) : data(value), num_sub_tree(1), left(nullptr), right(nullptr), parent(nullptr), height(0), balance_factor(0) {}
    ~AVLNode() = default;
    T data;
    int num_sub_tree;
    ...
    void print_node(int rank) const { std::cout << "Value = " << data << " , Rank = " << rank << std::endl; }
